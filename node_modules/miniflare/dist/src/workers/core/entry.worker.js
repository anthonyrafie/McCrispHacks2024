...
function getUserRequest(request, env) {
  ...
  return request = new Request(url, request), request.cf === void 0 && (request = new Request(request, { cf: env[CoreBindings.JSON_CF_BLOB] })), rewriteHeadersFromOriginalUrl && (request.headers.set("Host", url.host), request.headers.has("Origin") && request.headers.set("Origin", url.origin)), request.headers.delete(CoreHeaders.PROXY_SHARED_SECRET), request.headers.delete(CoreHeaders.ORIGINAL_URL), request.headers.delete(CoreHeaders.DISABLE_PRETTY_ERROR), request;
}
...
var entry_worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      return disablePrettyErrorPage || (response = await maybePrettifyError(request, response, env)), response = maybeInjectLiveReload(response, env, ctx), maybeLogRequest(request, response, env, ctx, startTime), response;
    } catch (e) {
